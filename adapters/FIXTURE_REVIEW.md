...
KiCad 10.0.6으로 기존 `balun_eth_rj45.kicad_pcb`를 검사했다. 표준 라이브러리를 연결한 환경에서 **DRC 0, 미연결 0, 회로도 parity 0**이다. 최초 검토에서는 copper/net을 유지했고, 후속 [임피던스 패치](../docs/jlcpcb/IMPEDANCE.md)에서 폭을 수정한 뒤 같은 검사를 다시 통과했다. 회로도 net은 유지했다.
...
- differential track은 0.234 mm trunk와 0.15 mm RJ45 escape로 구성된다. 0.15 mm 구간을 100 Ω 전송선으로 간주하지 않는다.
...
| JLC nominal geometry | 공식 계산기 반영 완료; 생산 CAM/coupon 및 실물 검증 남음 |
