...
현재 성숙도는 **Python 합성 검증 + 배선된 어댑터 CAD 검증**이다. 실물 RF 정확도, 커넥터 호환·패널 기구, JLC 최종 임피던스는 미검증이며, O/S/L/T 표준의 전용 PCB도 아직 없다.
...
| W 0.23 / gap 0.22 mm trunk | 100 Ω 차동을 목표로 한 nominal 출발값 | JLC 실제 stack/revision, solver와 coupon; escape/fanout은 목표 보장 범위 밖 |
...
아래는 구현 패치 당시 실행한 검증 범위다. 문서 후속 패치에서 RF 검증을 추가로 수행한 것은 아니다.
...
아직 없는 산출물은 **전용 O/S/L/T 표준 PCB, 실제 표준 모델의 측정/추정 근거, 불확도 예산, 실측 DUT 결과, JLC 최종 solver/coupon, 패널 제작 도면, 새 구성의 통합 구매 수량표와 어댑터 제조 release**다. 기존 `export_jlc_release.ps1`는 새 `adapters/`를 export하지 않는다.
