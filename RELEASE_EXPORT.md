...
> **Scope update, 2026-09-05:** The active design is the common RJ45 balun fixture plus passive adapters. Start with [DESIGN_REVIEW_HANDOFF](DESIGN_REVIEW_HANDOFF.md) and [adapter build notes](adapters/JLCPCB_BUILD.md). The exporter described below **does not export the new `adapters/` designs**. Its historical quantity/assembly overrides do not define the current system order. Existing export behavior is unchanged; no new adapter manufacturing release is provided by this documentation update.
